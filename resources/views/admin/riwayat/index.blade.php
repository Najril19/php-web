...
<div class="mx-auto max-w-6xl space-y-6">
    ...
    <form method="get" action="/admin/riwayat" class="flex flex-col gap-3 rounded-2xl border border-slate-200/90 bg-white p-4 shadow-[0_4px_24px_rgba(15,23,42,0.06)] sm:flex-row sm:flex-wrap sm:items-end">
        ...
        @if ($q !== '' || $tingkat !== '')
            <a href="/admin/riwayat" class="rounded-xl border border-slate-200 px-5 py-3 text-center text-sm font-semibold text-slate-600 hover:bg-slate-50">Reset</a>
        @endif
    </form>
            ...
            <tbody>
                @forelse ($rows as $i => $r)
                    ...
                    <tr class="border-t border-slate-100 odd:bg-white even:bg-slate-50/80">
                        ...
                        <td class="px-4 py-3.5 font-medium text-slate-900">{{ $namaUser[$r->id_user] ?? '—' }}</td>
                        ...
                        <td class="whitespace-nowrap px-4 py-3.5">
                            <a href="/admin/diagnosa/{{ $r->id }}" class="font-bold text-brand-600 hover:text-brand-700">Detail</a>
                            <form method="post" action="/admin/riwayat/hapus" class="ml-3 inline" onsubmit="return confirm('Hapus?');">@csrf<input type="hidden" name="id" value="{{ $r->id }}"><button type="submit" class="text-xs font-bold text-red-600 hover:underline">Hapus</button></form>
                        </td>
                    </tr>
                @empty
